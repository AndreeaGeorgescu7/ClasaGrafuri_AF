...
void GrafOrientat::BFord(int s,bool &ok,vector<int> &dist2, vector<vector<pair<int,int>>> &graf_c)
{
    ...
    while(!coada.empty() && !ok)
    {
        ...
        for(int i=0; i<graf_c[nod].size(); i++)
        {
            int vecin=graf_c[nod][i].first;
            int cost=graf_c[nod][i].second;


            if( dist2[nod] + cost < dist2[vecin])
            {
...
private:
    ...
    int tata[101], rang[101],st[101],dr[101];
public:
    GrafNeorientat():Graf() {}
    ~GrafNeorientat() {}
    ...
    void ciclu_eulerian(int s,int &poz,int muchii_viz[100],int drum[100],vector<pair<int,int>>v[100]);
    void afiseaza_euler(int poz,int drum[1000]);
...
void GrafNeorientat::ciclu_eulerian(int s,int &poz,int muchii_viz[100],int drum[100],vector<pair<int,int>>v[100])
{
    while(!v[s].empty())
    {
        ...
        {
            muchii_viz[y]=1;
            ciclu_eulerian(x,poz,muchii_viz,drum,v);
        }
    }
    drum[poz] = s;
    poz++;
...
/*void GrafNeorientat::afiseaza_euler(int poz,int drum[1000])
{
    for()
...
int main()
{
    ...
    else if(comanda==0)
    {
        ...
        while(k2==1)
        {
            ...
            case 11:
            {
                int i,poz=1,drum[100]= {0},muchii_viz[100]= {0};
                //memset(muchii_viz, 0, n);
                for(i=1; i<=n; i++)
                {
                    if(grad[i]%2==1)
                    {
